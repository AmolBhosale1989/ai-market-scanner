...
def detect_forming_setup(d: pd.DataFrame):
    ...
    if price<=0 or atr<=0:
        return {"formation_score":0,"stage":"REJECT","pattern":"insufficient data",
                "distance_to_20d_high_pct":0,"extension_above_20d_high_pct":0,
                "distance_to_ema50_pct":0,"distance_to_ema200_pct":0}
    ...
    extension=max(0.0,_pct(price,high20))
    if 0<=dist_res<=4:
    ...
    # Never call a stock "confirmed" after it has already run materially beyond
    # the breakout level. These are late entries, even if RVOL is huge.
    if extension>5 or (extension>3 and rsi>=78) or (extension>3 and rvol>=5):
        stage="EXTENDED"; tags.append("late/extended breakout")
    elif price>high20 and rvol>=1.3:
        stage="CONFIRMED"; tags.append("fresh breakout")
    ...
    return {
        ...
        "pattern":", ".join(tags[:6]) if tags else "none",
        "distance_to_20d_high_pct":round(dist_res,2),
        "extension_above_20d_high_pct":round(extension,2),
