    ...
    for n,ticker in enumerate(symbols,1):
        ...
        for i in range(MIN_HISTORY_DAYS,last_signal_index,stride):
            ...
            trades.append({
                ...
                "retest_quality_score":result.get("retest_quality_score",0),
                "ema20_slope5_pct":result.get("ema20_slope5_pct",math.nan),
                "support_touch_count":result.get("support_touch_count",0),
                "higher_low":result.get("higher_low",False),
                "bullish_close":result.get("bullish_close",False),
                "stop":round(stop,2),
